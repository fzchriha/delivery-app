...
export function BasketScreen(navigation) {
  ...
  const total = parseFloat(basketTotal) + deliveryFee;
  const [groupedItemsInBasket, setGroupedItemsInBasket] = useState([]);
  const dispatch = useDispatch();

  useEffect(() => {
    const groupedItems = items.reduce((results, item) => {
      // if item already exists push to items, if not make an empty array and push to it
      ...
      <View className="flex-1 bg-gray-100">
        ...
        <View className="p-5 bg-white mt-5 space-y-4">
          ...
          <TouchableOpacity
            ...
            onPress={() => {
              navigator.navigate(routes.PreparingOrder);
            }}
          >
